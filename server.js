...
// init sqlite db
const dbFile = "./.data/sqlite785.db";
...
db.serialize(() => {
  if (!exists) {
    db.run(
      "CREATE TABLE Items (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT, description TEXT, category int(22), contact TEXT, datejoined TEXT)"
    );
...
app.get("/getItems", (request, response) => {
  db.all("SELECT id,title,description, category, contact, datejoined from Items", (err, rows) => {
...
app.post("/addItem", (request, response) => {
  ...
  if (!process.env.DISALLOW_WRITE) {
    ...
    db.run(
      `INSERT INTO Items (title, description, category, contact, datejoined) VALUES (?,?,?,?,?)`,
      cleansedtitle,
      cleanseddescription,
      cleansedcategory,
      cleansedcontact,
